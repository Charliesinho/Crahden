...
// These two MUST come first, before any other require/code — if something
// throws or a promise rejects with nothing to catch it, Node can otherwise
// exit with zero output, which is exactly what "stops with no errors" looks
// like. Everything below this point is now guaranteed to at least log why.
process.on('uncaughtException', (err) => {
  console.error('UNCAUGHT EXCEPTION — server is exiting:', err);
  process.exit(1);
});
process.on('unhandledRejection', (reason) => {
  console.error('UNHANDLED PROMISE REJECTION — server is exiting:', reason);
  process.exit(1);
});
...
app.post('/api/creator/onboard', async (req, res) => {
  try {
    ...
    if (!accountId) {
      // Country is fixed permanently at account creation and defaults to
      // YOUR platform's country if not given explicitly — that's exactly
      // what was happening (a Latvian creator seeing France pre-filled).
      // Has to be captured here, before the account exists; nothing later
      // in onboarding can change it.
      const country = String(req.body.country || '').toUpperCase();
      if (!/^[A-Z]{2}$/.test(country)) {
        return res.status(400).json({ error: 'Please choose your country before continuing.' });
      }
    ...
    res.status(500).json({ error: err.message ? `Could not start onboarding: ${err.message}` : 'Could not start onboarding. Try again.' });
  }
});
...
async function start() {
  ...
  if (!process.env.MONGO_PASS) {
    console.error('FATAL: Missing MONGO_PASS in .env (or Railway\'s Variables tab) — the login system needs it to connect to MongoDB.');
    process.exit(1);
  }
  if (!stripe) {
  ...
  console.log(`Connecting to MongoDB (cluster: ${MONGO_CLUSTER}, db: ${MONGO_DB}, user: ${MONGO_USER})...`);
  try {
    // serverSelectionTimeoutMS forces this to actually FAIL within 15s
    // instead of retrying silently forever, which is exactly what "stops
    // with no errors" looks like — most likely cause is MongoDB Atlas's
    // Network Access list not including Railway's IP (Atlas → Network
    // Access → Add IP Address → Allow Access from Anywhere for Railway,
    // since it doesn't have a fixed outbound IP on most plans).
    ...
    console.error('Most likely causes: (1) MongoDB Atlas Network Access does not allow Railway\'s IP — add 0.0.0.0/0 there to test, (2) MONGO_PASS/MONGO_USER/MONGO_CLUSTER is wrong, (3) the database user does not have access to this cluster.');
    process.exit(1);
  }
  console.log('Connected to MongoDB.');

  await User.init(); // makes sure the unique index on usernameLower exists before we accept registrations
  console.log('User index ready.');
...
start().catch((err) => {
  console.error('FATAL: start() threw:', err);
  process.exit(1);
});